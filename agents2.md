...
@Component({
  ...
  template: `
    <a routerLink="/" class="inline-flex items-center text-blue-600 hover:text-blue-800">
      <svg class="w-5 h-5 mr-2"><!-- Arrow icon --></svg>
      Back to Home
    </a>
  `
})
...
constructor(private api: BookApiClient) {
  this.api.getBooks().subscribe(books => this.books = books); // ❌ DON'T
}
```

**Good:**
```typescript
constructor(private api: BookApiClient) {} // ✅ Only DI

ngOnInit(): void {
  this.loadBooks(); // ✅ Initialize in lifecycle hook
}
...
// Subscribe once in constructor or use async pipe in template
constructor() {
...
// Or even better: use async pipe in template
// Template: <div>{{ book$ | async }}</div>
```
